...
@pytest.mark.slow
@pytest.mark.requires_orbstack
@pytest.mark.parametrize("arch_flag,expected_nix_arch", [
    ("aarch64", "aarch64-linux"),
    ("arm64", "aarch64-linux"),
])
def test_explicit_arm_architecture(test_machine, project_root, test_username, arch_flag, expected_nix_arch):
...
def test_explicit_x86_architecture(test_machine, project_root, test_username, arch_flag, expected_nix_arch):
...
def test_flake_attribute_matches_architecture(test_machine, project_root, test_username):
    ...
    cmd = [
        "python3", str(provision_script),
        "create", machine_name,
        "--flake-attr", flake_attr,
        "--username", test_username,
    ]
    result = subprocess.run(cmd, capture_output=True, text=True, timeout=600)

    assert result.returncode == 0, f"Creation with flake attr {flake_attr} failed: {result.stderr}"
    assert machine_exists(machine_name)
